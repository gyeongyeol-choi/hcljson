...
func (p *printer) objectItem(o *ast.ObjectItem, typeSchema map[string]interface{}) []byte {
	...
	for i, k := range o.Keys {
		...
		// MEMO : 키워드 부분은 "" 제거해줌
		if i == 0 {
			rawKey = strings.Replace(k.Token.Text, "\"", "", -1)
		} else {
			rawKey = k.Token.Text
		}
		...
		if o.Assign.IsValid() && i == len(o.Keys)-1 && len(o.Keys) == 1 {
			// MEMO : object, block 타입일 땐 = 표시 없어야 됨. 그 외의 type들이나 typeSchema에 정의안된 필드들은 = 붙여줌.
			// MEMO : terraform keyword에 대한 HCL 역파싱 시 = 표시 없도록 수정
			if objectType != "object" && objectType != "block" && o.Keys[0].Token.Text != "\"terraform\"" {
				buf.WriteString("=")
			}
			buf.WriteByte(blank)
		}
